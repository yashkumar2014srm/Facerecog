...
while True:
    ...
    for(x,y,w,h) in faces:
        ...
        if(profile!=None):
            ...
            cv2.cv.PutText(cv2.cv.fromarray(im),"Subscriber Count: More Than "+str(profile[2]), (x,y+h+50),font, (0,0,255)) #Draw the text
            #cv2.cv.PutText(cv2.cv.fromarray(im),"Gender:"+str(profile[3]), (x,y+h+80),font, (0,0,255)) #Draw the text
            cv2.cv.PutText(cv2.cv.fromarray(im),"Criminal Records:"+str(profile[4]), (x,y+h+80),font, (0,0,255)) #Draw the text
    cv2.imshow('im',im)
    cv2.waitKey(10)
